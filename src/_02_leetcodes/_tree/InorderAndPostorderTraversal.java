...
public class InorderAndPostorderTraversal {
    ...
    //recursion
    public _02_leetcodes.TreeNode buildTree(int[] inorder, int[] postorder) {
        return buildTree(inorder, 0, inorder.length - 1, postorder, 0, postorder.length - 1);
    }
    private _02_leetcodes.TreeNode buildTree(int[] inorder, int inStart, int inEnd, int[] postorder, int postStart, int postEnd){
        if (inStart> inEnd || postStart > postEnd){
            return null;
        }

        int rootVal = postorder[postEnd];
        _02_leetcodes.TreeNode root = new TreeNode(rootVal);
